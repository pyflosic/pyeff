...
class eff_pbc_optimize:
	...
	def __init__(self,p_cfg,emax,fmax,steps,fix_nuc=None,method='BFGS'):
		...
		# pyeff calculation object 
                calc = eff_pbc(p_cfg=self.p_cfg,fix_nuc=self.fix_nuc)
                calc.initialize()
	...
	def write_input(self,x):
		...
		for p in range(len(self.types)):
				...
				o.write('%0.5f %0.5f %0.5f %s\n' %(wrap(x[idx+0],self.Lx),wrap(x[idx+1],self.Ly),wrap(x[idx+2],self.Lz),chemical_symbols[p]))
                                #o.write(str(x[idx+0])+' '+str(x[idx+1])+' '+str(x[idx+2])+' '+str(chemical_symbols[p])+'\n')
				idx = idx + 3 
                o.write('@electrons\n')
                for p in range(len(self.types)):
                        if self.types[p] == 'electron':
                                o.write('%0.5f %0.5f %0.5f %i %0.5f\n' %(wrap(x[idx+0],self.Lx),wrap(x[idx+1],self.Ly),wrap(x[idx+2],self.Lz),spins[p],np.exp(x[idx+3])))
	...
	def f(self,x):
		...
		self.write_input(x)
		calc = eff_pbc(p_cfg='eff.cfg',fix_nuc=self.fix_nuc)
		calc.initialize()
	...
	def write_xyz(self):
		...
		write('eff_movie.xyz',self.traj,'extxyz',write_results=False)
...
if __name__ == "__main__":
	...
	steps = 10000
	fix_nuc = 'True'
	calc = eff_pbc_optimize(p_cfg,emax,fmax,steps,fix_nuc,method='BFGS')	
	calc.run()
	#calc.view()
	calc.write_xyz()
	t = time.clock() - t0
